...
DJFMP_CHN_df <- DJFMP_df %>% 
  ...
  select(Location, SampleDate, ForkLength, WaterTemp, MethodCode) %>% 
...
ggplot(data = DJFMP_CHN_df, aes(x = SampleDate, y = ForkLength)) +
  geom_line() +
  labs(title = "Fork Length of Chinook Salmon in San Francisco Estuary (1976-2021)",
       x = "Date",
       y = "Fork Length (mm)")
```
...
**Multiple R-squared:** Our coefficient of determination ($R^2$) tells us that 7.75% of the variation in juvenile Chinook salmon fork lengths is explained by water temperature at the site. 
